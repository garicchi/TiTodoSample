...
//Rare exceptions to indicate a bug in the _titodosample code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_titodosample = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_titodosample]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.titodosample.module"
